...
__global__ void kernel_forward(const int B, const int T, const int C, const int H,
                               ...
                               F *__restrict__ const _y, float *__restrict__ const last_state, float *__restrict__ const new_state)
{
    const int b = blockIdx.x / H;
    const int h = blockIdx.x % H;
    const int i = threadIdx.x;
    int offset = b*C + h*_N_ + i;
    _w += h*_N_;
    _u += h*_N_;
    // printf("%d %f\n",offset+4, float(_k[offset+4]));
    ...
    for (int t = b*T*C + h*_N_ + i; t < (b+1)*T*C + h*_N_ + i; t += C)
    {
        ...
        for (int j = 0; j < _N_; j+=4)
        {
            ...
            float4 x;
            // printf("%d %d %f\n",b*T*C + h*_N_ + i,offset*_N_+j, state[0]);
            ...
            s.w = s.w * w_.w + x.w;
            if(new_state!=NULL){
                new_state[offset*_N_+j] = s.x;
                new_state[offset*_N_+j+1] = s.y;
                new_state[offset*_N_+j+2] = s.z;
                new_state[offset*_N_+j+3] = s.w;
            }
        }
        _y[t] = F(y);
    }
...
void cuda_backward(int B, int T, int C, int H, bf16 *r, bf16 *k, bf16 *v, float *w, float *ww, bf16 *u, bf16 *gy, bf16 *gr, bf16 *gk, bf16 *gv, bf16 *gw, bf16 *gu, float *last_state)
{
    assert(H*_N_ == C);
    assert(_N_%4 == 0);
    kernel_backward<<<dim3(B * H), dim3(_N_)>>>(B, T, C, H, r, k, v, w, ww, u, gy, gr, gk, gv, gw, gu, last_state);
}